...
int find_index(std::vector<int> vet, int x) {
...
// Dada a posição para iniciar a busca, qual a primeira pos em que aparece X?
int find_begin(std::vector<int> vet, int x, int start = 0) {
    for (int i = start; i < (int) vet.size(); ++i)
        if (vet[i] == x) 
            return i;
    return -1;
}
